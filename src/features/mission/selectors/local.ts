...
/**
 * Returns the list of participants for a given mission item id. If the list is
 * not explicity defined, it is implicitly treated as belonging to all indices.
 */
export const getParticipantsOfMissionItemId: AppSelector<
  MissionIndex[],
  [MissionItem['id']]
> = createSelector(
  getMissionItemById,
...
export const getParticipantsForMissionItemIds: AppSelector<
  Record<MissionItem['id'], MissionIndex[]>
> = (state) =>
  Object.fromEntries(
...
> = createSelector(
  ...
  selectMissionIndex,
  (progressData, missionIndex) => progressData[missionIndex]?.currentItemId
);
...
> = createSelector(
  ...
  getMissionProgressData,
  (mapping, progress) =>
    mapping.map((_uavId, missionIndex) => progress[missionIndex]?.currentItemId)
);
...
> = createSelector(
  ...
  (ids, currentId) =>
    // TODO: Why not just let `indexOf` return `-1` for `undefined`?
...
> = createSelector(
  getMissionItemIds,
  getCurrentMissionItemIdForEveryMissionIndex,
  (missionItemIds, currentItemIds) =>
    currentItemIds.map((currentItemId) =>
      currentItemId === undefined
        ? undefined
        : missionItemIds.indexOf(currentItemId)
    )
);
...
> = createSelector(
  getMissionProgressData,
  selectMissionIndex,
  (progressData, missionIndex) => progressData[missionIndex]?.currentItemRatio
);
...
> = createSelector(
  ...
  (mapping, progress) =>
    mapping.map(
      (_uavId, missionIndex) => progress[missionIndex]?.currentItemRatio
    )
);
...
> = createSelector(
  getMissionProgressData,
  selectMissionIndex,
  (progressData, missionIndex) => !isNil(progressData[missionIndex])
);
