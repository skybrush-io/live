...
const from3Dto2D: (c: Coordinate3D) => Coordinate2D = ([x, y, _z]) => [x, y];
...
export const getConvexHullOfTrajectory = (
  ...
  convexHull2D(getPointsOfTrajectory(trajectory).map(from3Dto2D));
...
export const getMaximumHorizontalDistanceFromTakeoffPositionInTrajectory = (
  ...
  const distanceToFirstPoint = (keyframe: TrajectorySegment): number => {
    ...
    return euclideanDistance2D(from3Dto2D(point), from3Dto2D(firstPoint));
  };
